...
void zadatak1()
{
	...
	//p2->Prikazi(); 
	cout << *p2 << endl;
	double tezine[] = { 1.5, 2.7, 0.8, 3.2, 1.8 };
	double duzina[] = { 1.2, 3.1, 2.5, 3.4, 4.1 };
	int kod[] = { 100, 20, 40, 50, 30 };
	IspitPriprema* pIspit = new IspitPriprema(20);
	// 2 poena 
	for (int i = 0; i < 5; i++)
	{
		Lekcija* lekcija = new Lekcija(2 * i + 1, tezine[i], duzina[i]); pIspit->Dodaj(lekcija);
		Zadatak* zadatak = new Zadatak(2 * i + 2, tezine[i], kod[i]); pIspit->Dodaj(zadatak);
	}
	// 2 poena 
	//pe.print(); 
	cout << *pIspit << endl;
...
void zadatak2() {
	...
	try
	{
		float tezine[] = { 1.5, 2.7, 3.2, 0.8, 1.8, 4.1, 2.5, 2.3, 3.6 }; Ispit<float> ispit(maxPodataka);
		// 1 bod 
		...
		int broj = ispit.UkupniBrojVrednost(vrednost, ukupnaVrednost); cout << broj << " " << ukupnaVrednost << endl; // 4 11.8 
		// 1 bod 
		...
		// 1 boda 
		//Ispit<int> ispUc(maxPodataka); ovo su oni napisali ali je greska <int> ne moze bez kastovanja a onda gubi poentu funkcija
	...
	try
	{
		char naziv[][8] = { "prva", "druga", "treca", "cetvrta", "peta", "sesta", "sedma", "osma", "deveta" };
		float tezine[] = { 1.5, 2.7, 3.2, 0.8, 1.8, 4.1, 2.5, 2.3, 3.6 }; float duzina[] = { 1.2, 3.1, 2.5, 3.4, 4.1, 1.7, 2.2, 3.7, 2.8 }; Ispit<LekcijaNormalna> ispNor(maxPodataka);
		// 1 bod 
		for (int i = 0; i < brojPodataka; i++) {
		...
		// 2 bod 
		float vrednost = 2.0; float ukupnaVrednost = 0;
		int broj = ispNor.UkupniBrojVrednost(vrednost, ukupnaVrednost); cout << broj << " " << ukupnaVrednost << endl;
		// 2 bod 
